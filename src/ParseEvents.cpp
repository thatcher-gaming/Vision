...
bool ServerAgent::ParseEvents(const char* data)
{
	...
	if (secondWord == "PRIVMSG") {
		BString theNick(GetNick(data)), ident(GetIdent(data)), address(GetAddress(data)), addy;

		addy += ident;
		addy += "@";
		addy += address;

		BString theTarget(GetWord(data, 3).ToUpper()), theTargetOrig(GetWord(data, 3)),
	...
	if (secondWord == "NOTICE") {
		...
		if (firstWord.ICompare(fServerHostName) == 0) {
			...
			BString theNick(GetNick(data)), ident(GetIdent(data)), address(GetAddress(data));
	...
	if (secondWord == "JOIN") {
		BString nick(GetNick(data)), channel(GetWord(data, 3));
		...
		if (nick == fMyNick) {
			...
			// someone else
			BString ident(GetIdent(data)), address(GetAddress(data)), tempString;
			...
			PackDisplay(&display, tempString.String(), C_JOIN);
	...
	if (secondWord == "PART") {
		BString nick(GetNick(data)), channel(GetWord(data, 3)), reason(RestOfString(data, 4));
		...
		if ((client = Client(channel.String())) != 0) {
			BString ident(GetIdent(data)), address(GetAddress(data)), buffer;
	...
	if (secondWord == "NICK") {
		BString oldNick(GetNick(data)), ident(GetIdent(data)), address(GetAddress(data)),
			newNick(GetWord(data, 3)), buffer;
	...
	if (secondWord == "QUIT") {
		BString theNick(GetNick(data).String()), theRest(RestOfString(data, 3)),
			ident(GetIdent(data)), address(GetAddress(data)), theMsg, firstNick;
	...
	if (firstWord == "PING") {
		BString tempString, theServer(GetWord(data, 2));
	...
	if (secondWord == "KICK") {
		BString kicker(GetNick(data)), kickee(GetWord(data, 4)), rest(RestOfString(data, 5)),
		...
		if ((client = Client(channel.String())) != 0 && kickee == fMyNick) {
	...
	if (secondWord == "TOPIC") {
		BString theNick(GetNick(data)), theChannel(GetWord(data, 3)),
	...
	if (secondWord == "MODE") {
		BString theNick(GetNick(data)), theChannel(GetWord(data, 3)), theMode(GetWord(data, 4)),
	...
	if (secondWord == "WALLOPS") {
		BString theNick(GetNick(data)), theWall(RestOfString(data, 3)), tempString;
	...
	if (secondWord == "INVITE") {
		BString tempString, theChannel(GetWord(data, 4));
